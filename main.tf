...
create{
statements = "use role accountadmin;GRANT OWNERSHIP ON ROLE tharunsnow TO ROLE ACCOUNTADMIN;GRANT USAGE ON DATABASE DB_DG TO ROLE tharunsnow;GRANT USAGE ON SCHEMA DB_DG.SCH_DG TO ROLE tharunsnow;USE ROLE ACCOUNTADMIN;CREATE table db_dg.sch_dg.testtable(col varchar(20));"
}
delete {
    statements = "select 1;"
  }
